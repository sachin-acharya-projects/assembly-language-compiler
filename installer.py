...
if os.path.exists(FILENAME):
    print(
        f"{Fore.GREEN}Required File Already Exist. Deleting the Old file and downloading new file"
    )
    os.remove(FILENAME)
...
if os.path.exists(FOLDER):
    ...
    print(
        f"{Fore.LIGHTGREEN_EX}Press ENTER to Delete Old Folder and Create New Folder with New Data"
    )
...
try:
    subprocess.check_output(
        "cat ~/.bashrc | grep 'alias assembly_compiler'", shell=True
    )
...
print(
    f"""{Fore.LIGHTMAGENTA_EX}Installation has been completed
